...
if [[ -z "$monitor_password" || "$monitor_password" == "None" ]]; then
  monitor_password="$(openssl rand -hex 24)"
  aws secretsmanager put-secret-value \
    --region "$AWS_REGION" \
    --secret-id "$MONITOR_SECRET_ID" \
    --secret-string "$monitor_password" >/dev/null
  log "Generated and stored a new Monitor admin password in Secrets Manager"
else
  log "Reusing the existing Monitor admin password from Secrets Manager"
fi
